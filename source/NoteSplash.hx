...
class NoteSplash extends FlxSprite
{
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null):Void
	{
		...
		if (note != null)
		{
			...
			else if (PlayState.SONG.splashSkin != null && PlayState.SONG.splashSkin.trim().length > 0) {
		...
		{
			if (note != null && !note.noteSplashData.useGlobalShader) // If Note RGB is enabled:
			{
	...
	public static function getSplashSkinPostfix():String
	{
		...
		if (ClientPrefs.splashSkin != ClientPrefs.defaultData.splashSkin) {
	...
	public static function precacheConfig(skin:String):NoteSplashConfig
	{
		...
		var path:String = Paths.getTxt('images/$skin');
...
class PixelSplashShaderRef
{
	...
	public function copyValues(tempShader:RGBPalette):Void
	{
