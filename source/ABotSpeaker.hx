...
class ABotSpeaker extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0):Void
	{
		...
		bg.loadGraphic(Paths.getImage('abot/stereoBG'));
	...
	override function update(elapsed:Float):Void
	{
		...
		for (i in 0...Std.int(Math.min(vizSprites.length, levels.length)))
		{
			...
			animFrame = Std.int(Math.abs(CoolUtil.boundTo(animFrame, 0, 5) - 5)); // shitty dumbass flip, cuz dave got da shit backwards lol!
