...
class ModsMenuState extends MusicBeatState
{
	...
	public function new(startMod:String = null):Void
	{
	...
	override function create():Void
	{
		...
		buttonEnableAll.focusChangeCallback = function(focus:Bool):Void if (!focus) buttonEnableAll.bg.color = FlxColor.GREEN;
		add(buttonEnableAll);

		buttonDisableAll = new MenuButton(buttonX, myY, buttonWidth, buttonHeight, "DISABLE ALL", function():Void
		{
		...
		if (modsList.all.length < 1)
		{
			...
			var txt:FlxText = new FlxText(bgList.x + 15, bgList.y + 15, bgList.width - 30, "No Mods found.", 16);
			txt.setFormat(Paths.getFont("vcr.ttf"), 16, FlxColor.WHITE);
			add(txt);
		...
		modDesc.setFormat(Paths.getFont("vcr.ttf"), 24, FlxColor.WHITE, LEFT);
		add(modDesc);
		...
		modRestartText.setFormat(Paths.getFont("vcr.ttf"), 16, FlxColor.WHITE, RIGHT);
		add(modRestartText);
		...
		var button:MenuButton = new MenuButton(buttonsX, buttonsY, 80, 80, Paths.getImage('modsMenuButtons'), function() moveModToPosition(0), 54, 54); //Move to the top
		...
		var button:MenuButton = new MenuButton(buttonsX + 100, buttonsY, 80, 80, Paths.getImage('modsMenuButtons'), function() moveModToPosition(curSelectedMod - 1), 54, 54); //Move up
		...
		var button:MenuButton = new MenuButton(buttonsX + 200, buttonsY, 80, 80, Paths.getImage('modsMenuButtons'), function() moveModToPosition(curSelectedMod + 1), 54, 54); //Move down
		...
		buttons.push(button);
		button.focusChangeCallback = function(focus:Bool):Void
		{
			if (!focus) {
	...
	override function update(elapsed:Float):Void
	{
		...
		if (modsList.all.length > 0)
		{
			...
			if (modsList.all.length > 1)
			{
				...
				if (hoveringOnMods)
				{
					...
					else if (FlxG.keys.justPressed.HOME || FlxG.keys.justPressed.END || FlxG.gamepads.anyJustPressed(LEFT_TRIGGER) || FlxG.gamepads.anyJustPressed(RIGHT_TRIGGER))
					{
		...
		else
		{
			...
			if (nextAttempt < 0)
			{
				...
				if (modsList.all.length > 0)
				{
					Debug.logInfo('mod(s) found! reloading');
					reload();
				}
			}
		}
	...
	function changeSelectedButton(add:Int = 0):Void
	{
	...
	function changeSelectedMod(add:Int = 0, isMouseWheel:Bool = false):Void
	{
	...
	function updateModDisplayData():Void
	{
		var curMod:ModItem = modsGroup.members[curSelectedMod];
	...
	function updateItemPositions():Void
	{
		...
		for (i => mod in modsGroup.members)
		{
			if (mod == null)
			{
				Debug.logInfo('Mod #$i is null, maybe it was ' + modsList.all[i]);
				continue;
			}
	...
	function moveModToPosition(?mod:String = null, position:Int = 0):Void
	{
		...
		Debug.logInfo('Moved mod $mod to position $position');
	...
	function checkToggleButtons():Void
	{
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String):Void
	{
		...
		text.setFormat(Paths.getFont("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.borderSize = 2;
		text.y -= Std.int(text.height / 2);
		add(text);

		var isPixel:Bool = false;
		var bmp:BitmapData = BitmapData.fromFile(Paths.mods('$folder/pack.png'));

		if (bmp == null)
		{
			bmp = BitmapData.fromFile(Paths.mods('$folder/pack-pixel.png'));
			isPixel = true;
		}
		...
		else icon.loadGraphic(Paths.getImage('ui/unknownMod'), true, 150, 150);
...
class MenuButton extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, width:Int, height:Int, ?text:String = null, ?img:FlxGraphic = null, onClick:Void->Void = null, animWidth:Int = 0, animHeight:Int = 0):Void
	{
		super(x, y);
	...
	public function setButtonVisibility(focusVal:Bool):Void
	{
		alpha = 1;
		bg.color = focusVal ? FlxColor.WHITE : FlxColor.BLACK;
		bg.alpha = focusVal ? 0.8 : 0.6;

		var focusAlpha:Float = focusVal ? 1 : 0.6;
	...
	public function centerOnBg(spr:FlxSprite):Void
	{
