...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	function reloadSelectedCharacter():Void
	{
		...
		var imagePath:String = 'storymenu/menucharacters/' + characterFile.image;

		if (Paths.fileExists('images/menucharacters/' + characterFile.image + '.png', TEXT)) {
			imagePath = 'menucharacters/' + characterFile.image;
		}
	...
	/**
		...
		*/
	function onLoadCancel(_:Event):Void
	{
		_file.removeEventListener(#if desktop Event.SELECT #else Event.COMPLETE #end, onLoadComplete);
		_file.removeEventListener(Event.CANCEL, onLoadCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
	...
	/**
		...
		*/
	function onLoadError(_:Event):Void
	{
		_file.removeEventListener(#if desktop Event.SELECT #else Event.COMPLETE #end, onLoadComplete);
		_file.removeEventListener(Event.CANCEL, onLoadCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
