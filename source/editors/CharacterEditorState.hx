...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create():Void
	{
		...
		var dad:Sprite = new Sprite(dadPosition.x, dadPosition.y);
		dad.loadGraphic(Paths.getImage('editors/silhouetteDad'));
		dad.antialiasing = ClientPrefs.globalAntialiasing;
		dad.active = false;
		dad.offset.set(-4, 1);
		silhouettes.add(dad);

		var boyfriend:Sprite = new Sprite(bfPosition.x, bfPosition.y + 350);
		boyfriend.loadGraphic(Paths.getImage('editors/silhouetteBF'));
		...
		cameraFollowPointer.loadGraphic(Paths.getImage('cursorCross', 'embed'));
	...
	function addHelpScreen():Void
	{
		...
		helpBg = new Sprite();
		helpBg.makeGraphic(1, 1, FlxColor.BLACK);
	...
	function addGhostUI():Void
	{
		...
		highlightGhost.callback = function():Void
		{
		...
		ghostAlphaSlider.callback = function(relativePos:Float):Void
		{
			ghost.alpha = ghostAlpha;
	...
	function addPropertiesUI():Void
	{
		...
		characterDeathName = new FlxUIInputText(15, 35, 150, character.deathChar, 8);
		characterDeathSound = new FlxUIInputText(characterDeathName.x, characterDeathName.y + 45, 150, character.deathSound, 8);
		characterDeathConfirm = new FlxUIInputText(characterDeathName.x, characterDeathName.y + 85, 150, character.deathConfirm, 8);
		characterDeathMusic = new FlxUIInputText(characterDeathName.x, characterDeathName.y + 125, 150, character.deathMusic, 8);
	...
	function addSettingsUI():Void
	{
		...
		charDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(index:String)
		{
			...
			var path:String = Paths.getFile(characterPath, TEXT);
	...
	function addAnimationsUI():Void
	{
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 60, "Add/Update", function():Void
		{
		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 60, "Remove", function():Void
		{
			for (anim in character.animationsArray)
			{
				if (animationInputText.text == anim.anim)
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		if (sender is FlxUIInputText)
		{
			...
			{
				var lastIcon:String = healthIcon.character;
				...
				character.deathChar = characterDeathName.text;
			}
			else if (sender == characterDeathSound) {
				character.deathSound = characterDeathSound.text;
			}
			else if (sender == characterDeathConfirm) {
				character.deathConfirm = characterDeathConfirm.text;
			}
			else if (sender == characterDeathMusic) {
				character.deathMusic = characterDeathMusic.text;
			}
		}
	...
	function reloadCharacterImage():Void
	{
		...
		if (Paths.fileExists('images/' + character.imageFile + '/Animation.json', TEXT))
		{
			...
			try {
				...
				Debug.logWarn('Could not load atlas ${character.imageFile}: $e');
			}
	...
	function reloadCharacterOptions():Void
	{
		...
		characterDeathName.text = character.deathChar;
		characterDeathConfirm.text = character.deathConfirm;
		characterDeathMusic.text = character.deathMusic;
	...
	override function update(elapsed:Float):Void
	{
		...
		var changedAnim:Bool = false; // CHARACTER CONTROLS
	...
	inline function updateTextColors():Void
	{
		var daLoop:Int = 0;
	...
	{
		characterList = Paths.mergeAllTextsNamed('data/characterList.txt', Paths.getPreloadPath());

		#if MODS_ALLOWED
		var foldersToCheck:Array<String> = Paths.directoriesWithFile([Paths.getPreloadPath()], 'characters/');
	...
	function onSaveError(_:Event):Void
	{
		...
		Debug.logInfo("Problem saving file");
	}
	...
	function saveCharacter():Void
	{
		...
		var json:Dynamic = {
			"gameover_properties": [
				...
				character.deathMusic,
				character.deathConfirm
			],
			"skip_dance": character.skipDance,
